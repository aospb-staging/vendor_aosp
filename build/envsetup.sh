...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/aosPB-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.aosp.device | grep -q "$AOSP_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/aosPB-Project/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.aosp.device | grep -q "$AOSP_BUILD");
    then
